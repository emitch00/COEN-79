...
namespace coen79_lab8
{
    ...
    void deque<Item>::push_front(const value_type& entry)
    {
        ...
        {
            value_type *tmp_block_ptr = new value_type[++bp_array_size];
            *tmp_block_ptr = *new value_type[block_size];
            --first_bp;
            *first_bp= tmp_block_ptr;
            **first_bp= *front_ptr= entry;
        ...
        {
            value_type *tmp_block_ptr = new value_type[++bp_array_size];
            for (size_type i = 0; i<bp_array_size-1; ++i) {
                tmp_block_ptr[i+1]=block_pointers[i];
            }
            
            *tmp_block_ptr = new value_type[block_size];
            size_type diff = last_bp - first_bp;
            last_bp = first_bp + diff  +1;
            first_bp = tmp_block_ptr;
            front_ptr = *first_bp + block_size-1;
            *front_ptr = entry;
            
            delete [] block_pointers;
            block_pointers = tmp_block_ptr;
        }
    }
    ...
    void deque<Item>::push_back(const value_type& entry)
    {
        ...
        else if ((back_ptr == ((*last_bp) + (block_size - 1))) && (last_bp != block_pointers_end))
        {
            value_type *tmp_block_ptr = new value_type[++bp_array_size];
            *tmp_block_ptr = new value_type[block_size];
            ++last_bp = tmp_block_ptr;
            *last_bp= back_ptr= entry;
        }
        ...
        else if ((back_ptr == ((*last_bp) + (block_size - 1))) && (last_bp == block_pointers_end))
        {
            value_type *tmp_block_ptr = new value_type[++bp_array_size];
            for (size_type i = 0; i<bp_array_size-1; ++i) {
                tmp_block_ptr[i]=block_pointers[i+1];
            }
    ...
    void deque<Item>::pop_front()
    {
        ...
        {
            size_type diff = first_bp - block_pointers;
            delete[] block_pointers[diff];
            first_bp++;
            front_ptr=*first_bp;
        }
